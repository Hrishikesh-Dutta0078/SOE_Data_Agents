  ...
  return (
    <div className="flex flex-col h-full text-xs">
      <div className="flex-1 overflow-auto">
              ...
              <tr>
                {cols.map((col) => (
                  <th
                    ...
                    className="px-2.5 py-1.5 text-left text-[11px] font-semibold text-slate-600 uppercase tracking-wide border-b border-slate-200 cursor-pointer hover:bg-slate-100 select-none whitespace-nowrap"
                  >
                    {col}
                    {sortKey === col && (
            ...
            <tbody>
              {displayedRows.map((row, i) => (
                <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                  {cols.map((col) => (
                    <td key={col} className="px-2.5 py-1.5 text-slate-700 whitespace-nowrap">
        ...
        {error && (
          <div className="px-2.5 py-1.5 text-[11px] text-rose-600">{error}</div>
        )}
      </div>

      {/* Pagination controls */}
      <div className="flex items-center justify-between px-2.5 py-1.5 border-t border-slate-200 bg-slate-50 text-[11px] text-slate-500">
        <span>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="px-2 py-0.5 rounded border border-slate-200 bg-white hover:bg-slate-100
                         disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer text-slate-600 text-[11px]"
            >
              Prev
            </button>
